  ...
  pathsForJsonPath = jsonPath:
    let
      ...
      createMavenPaths = name: { artifactId, groupId, version, classifier, sha512, ... }:
        ...
        in [
          {
            # TODO: Classifiers should have the added -${classifier} suffix, but that doesn't appear to work just like that
      ...
      */
      ...
      createGitPaths = name: { url, cleanUrl, rev, sha256, ... }: [
        {
          name = ".gitlibs/${cleanUrl}/config";
          path = pkgs.emptyFile;
        }
        {
          name = ".gitlibs/libs/${name}/${rev}";
          path = pkgs.fetchgit {
    ...
    in pkgs.linkFarm "clojure-deps-${baseNameOf jsonPath}" (lib.concatLists (lib.mapAttrsToList createPackagePaths jsonValue.packages));

in {
  homeDir = pkgs.buildEnv {
    name = "clojure-deps";
    paths = map pathsForJsonPath jsonPaths;
  };
}
